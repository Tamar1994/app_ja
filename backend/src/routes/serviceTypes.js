...
function parsePriceTiers(rawValue) {
  ...
  if (typeof rawValue === 'string') {
    try { parsed = JSON.parse(rawValue); } catch {
      throw Object.assign(new Error('priceTiers: JSON inválido'), { status: 400 });
    }
  }
  ...
  return parsed.map((t, idx) => {
    ...
    if (!label) throw Object.assign(new Error(`priceTiers[${idx}]: label é obrigatório`), { status: 400 });
    if (!Number.isFinite(durationMinutes) || durationMinutes < 1) throw Object.assign(new Error(`priceTiers[${idx}]: durationMinutes inválido`), { status: 400 });
    if (!Number.isFinite(price) || price < 0) throw Object.assign(new Error(`priceTiers[${idx}]: price inválido`), { status: 400 });
...
function parseUpsells(rawValue) {
  ...
  if (typeof rawValue === 'string') {
    try { parsed = JSON.parse(rawValue); } catch {
      throw Object.assign(new Error('upsells: JSON inválido'), { status: 400 });
    }
  }
  ...
  return parsed.map((u, idx) => {
    ...
    if (!key) throw Object.assign(new Error(`upsells[${idx}]: key é obrigatório`), { status: 400 });
    if (!label) throw Object.assign(new Error(`upsells[${idx}]: label é obrigatório`), { status: 400 });
    if (!Number.isFinite(price) || price < 0) throw Object.assign(new Error(`upsells[${idx}]: price inválido`), { status: 400 });
...
const serviceTypeUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    else cb(new Error('Envie um ícone PNG, WEBP ou JPG'));
  },
...
router.delete('/:id', adminAuth, async (req, res) => {
  try {
    ...
    if (!deleted) return res.status(404).json({ message: 'Tipo não encontrado' });
